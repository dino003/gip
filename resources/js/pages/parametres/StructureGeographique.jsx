...
import StructureGeographiqueItem from '../../components/codifications/StructureGeographiqueItem';

  class StructureGeographique extends Component {
    ...
    onDelete = (id) => {
        ...
        if(conf === true){
            ...
            const action = {type: "REMOVE_STRUCTURE_GEOGRAPHIQUE", value: id}
    ...
    onDelete = (item) => {
        ...
        let conf = confirm('Voulez-vous vraiment supprimer ?')
        if(conf === true){
            if(item.plan_geographiques && item.plan_geographiques.length){
                let message = `${item.libelle} contient des éléments dans le plan Géographique
                en le supprimant cela supprimera toutes ses ramifications dans le plan Géographique
                Souhaitez-vous continuer ?`

                if(confirm(message) ){
                    
                   axios.delete('/api/supprimer_structure_geographique/' + item.id)
                 /*   item.plan_geographiques.forEach(element => {
                    axios.delete('/api/supprimer_plan_geographique/' + element.id)
                   }); */

                   const action = {type: "REMOVE_STRUCTURE_GEOGRAPHIQUE", value: item.id}
                    this.props.dispatch(action)
    ...
    onEditSubmit(libelle, niveau, id){
        ...
        axios.post('/api/modifier_structure_geographique/' + id, {
            libelle: libelle,
            niveau: niveau
        }).then(response => {
            const action = {type: "EDIT_STRUCTURE_GEOGRAPHIQUE", value: response.data}
            this.props.dispatch(action)
   ...
   // console.log(this.libelle.value)
    ...
    renderList(){
        ...
        <tr>
            ...
            <th>Libéllé de la Structure Géographique</th>
     ...
     { this.props.structure_geographiques.sort((a, b) => a - b.niveau).map((item, index) => 
         <StructureGeographiqueItem
         index={index}
          key={item.id} 
    ...
    render() {
        // console.log(Array.isArray(this.props.structure_geographiques))
        return (
                       ...
                       <div className="card-body ">
                          ...
                          {this.lastLevel() < 6 ?
                           ...
                           {this.state.loading ? this.renderLoading() : 
                            !this.props.structure_geographiques.length ? this.renderEmpty() : this.renderList()}
...
const mapStateToProps = state => {
    return {
        ...
        loading: state.structure_geographiques.loading,

    }
  }

export default connect(mapStateToProps)(StructureGeographique)
