...
import PlanOrganisationnelItem from '../../components/codifications/PlanOrganisationnelItem'

import { Container, Button, Link } from 'react-floating-action-button'
import ReactHTMLTableToExcel from 'react-html-table-to-excel';


 class PlanOrganisationnel extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            premier_niveau_structure_organisationnel: this.props.structure_organisationnelles[0] || null,
    ...
    ajouterEnfant(element){
      var structure_organisationnel = element.structure_organisationnel ? element.structure_organisationnel : this.props.structure_organisationnelles.find(st => st.id == element.structure_organisationnel_id)
        var structure_suivante = this.props.structure_organisationnelles.find(st => st.niveau == Number(structure_organisationnel.niveau + 1))
        this.setState({
    ...
    onEditPlanEnfant(element){
        var structure_organisationnel = element.structure_organisationnel ? element.structure_organisationnel : this.props.structure_organisationnelles.find(st => st.id == element.structure_organisationnel_id)
        var structure_suivante = this.props.structure_organisationnelles.find(st => st.niveau == Number(structure_organisationnel.niveau ))
        this.setState({
    ...
    onSupprimerPlan(element){
        ...
        if(conf === true){
            if(element.children && element.children.length){
                ...
                if(confirm(message) ){
                    axios.delete('/api/supprimer_plan_organisationelle/' + element.id).then( _ => {
    ...
    isAjoutPossible(element){
            ...
            var structure_correspondante = this.props.structure_organisationnelles.find(st => st.id == element.structure_organisationnel_id)
   ...
   async handleSubmitFormPlanEnfant(){
        if(this.verificationFormulairePlanEnfant() == null){
           ...
           await axios.post('/api/ajouter_plan_organisationelle', 
               {
                   libelle: this.libelle_nouveau_plan_enfant.value,
                   structure_organisationnel_id: this.state.prochain_type.id,
    ...
    onEditSubmitPlanEnfant = (e) => {
      ...
      this.setState({isFormSubmitted: true})
        axios.post('/api/modifier_plan_organisationelle/' + modif.id, {
    ...
    renderlist(){
          ...
          <tbody>
           {this.props.plan_organisationnels.map((st, index) => 
               <PlanOrganisationnelItem
                key={st.id} 
                index={index}
                item={st}
                onEdit={this.onEdit}
      ...
      renderTree(){
        ...
        return (
         ...
         <OptionsList 
         options={this.props.plan_organisationnels.filter(plan => plan.parent == null)} 
        ...
        return (
            ...
            <div className="main-card card" >
                       <div className="card-body ">
                           ...
                           <h5 className="card-title">Gestion du plan Organisationnel
                        ...
                        <form ref={(ref) => this.formRef = ref} className="p-3" onChange={this.setField} onSubmit={this.enregistrerPlanOrganisationnel}>
                            <br />
                                ...
                                <div className="position-relative form-group">
                                    ...
                                    readOnly
                                    value={this.state.premier_niveau_structure_organisationnel.libelle}
                                       type="text" className="form-control" />
                                    </div>
...
export default connect(mapStateToProps)(PlanOrganisationnel)
