...
import PlanBudgetaireItem from '../../components/codifications/PlanBudgetaireItem'

import { Container, Button, Link } from 'react-floating-action-button'
import ReactHTMLTableToExcel from 'react-html-table-to-excel';


 class PlanBudgetaire extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            premier_niveau_structure_budgetaire: this.props.structure_budgetaires[0] || null,
            isAjouterEnfant: false,
            prochain_type: null,
            libelle_nouveau_plan_enfant: null,
            code_nouveau_plan_enfant: null,
            parent_id: null,
            isEditEnfant: false,
            prochain_type_edit: null,
            libelle_nouveau_plan_enfant_edit: null,
            code_nouveau_plan_enfant_edit: null,
            parent_id_edit: null,
    ...
    verificationFormulairePlanEnfant () {
        if(this.libelle_nouveau_plan_enfant.value == '' ){
            return "Le Libéllé est obligatoire !"
        } else if(this.code_nouveau_plan_enfant.value == '' ){
            return "Le code est obligatoire !"
        } else{
            return null
        }
    ...
    ajouterEnfant(element){
      //  var plan = this.props.plan_budgetaires.find(pl => pl.id == element.id)
      var structure_budgetaire = element.structure_budgetaire ? element.structure_budgetaire : this.props.structure_budgetaires.find(st => st.id == element.structure_budgetaire_id)
        var structure_suivante = this.props.structure_budgetaires.find(st => st.niveau == Number(structure_budgetaire.niveau + 1))
        this.setState({
    ...
    onEditPlanEnfant(element){
        var structure_budgetaire = element.structure_budgetaire ? element.structure_budgetaire : this.props.structure_budgetaires.find(st => st.id == element.structure_budgetaire_id)
        var structure_suivante = this.props.structure_budgetaires.find(st => st.niveau == Number(structure_budgetaire.niveau ))
        this.setState({
            objetEditPlanEnfant: element,
            prochain_type_edit: structure_suivante,
            libelle_nouveau_plan_enfant_edit: element.libelle,
            code_nouveau_plan_enfant_edit: element.code,
        ...
        if(conf === true){
            if(element.children && element.children.length){
                ...
                if(confirm(message) ){
                  ...
                  axios.delete('/api/supprimer_plan_budgetaire/' + element.id).then( _ => {
    ...
    isAjoutPossible(element){
            ...
            var structure_correspondante = this.props.structure_budgetaires.find(st => st.id == element.structure_budgetaire_id)
   ...
   async handleSubmitFormPlanEnfant(){
        if(this.verificationFormulairePlanEnfant() == null){
          var parent_direct = this.props.plan_budgetaires.find(pl => pl.id == this.state.parent_id)
          var niveau_actuel = parent_direct.structure_budgetaire.niveau
          var objet_a_envoyer = {}

          this.setState({isFormSubmitted: true})

             // const struc = this.props.structures_etablissements.find(st =>)
           await axios.post('/api/ajouter_plan_budgetaire',
               {
                   libelle: this.libelle_nouveau_plan_enfant.value,
                   code: this.code_nouveau_plan_enfant.value,

                   structure_budgetaire_id: this.state.prochain_type.id,
              ...
              if(niveau_actuel == 1){
                ...
                var pre_niveau1 = this.props.plan_budgetaires.find(pl => pl.id == this.state.parent_id)
               var premier_niveau = this.props.plan_budgetaires.find(pl => pl.id == pre_niveau1.parent)
                objet_a_envoyer = {
                ...
                var pre_niveau2 = this.props.plan_budgetaires.find(pl => pl.id == this.state.parent_id)
                var deuxieme_niveau = this.props.plan_budgetaires.find(pl => pl.id == pre_niveau2.parent)
                var troisieme_niveau = this.props.plan_budgetaires.find(pl => pl.id == deuxieme_niveau.parent)
                ...
                var deuxieme_niveau = this.props.plan_budgetaires.find(pl => pl.id == pre_niveau2.parent)
                var troisieme_niveau = this.props.plan_budgetaires.find(pl => pl.id == deuxieme_niveau.parent)
                var quatrieme_niveau = this.props.plan_budgetaires.find(pl => pl.id == troisieme_niveau.parent)
                ...
                var troisieme_niveau = this.props.plan_budgetaires.find(pl => pl.id == deuxieme_niveau.parent)
                var quatrieme_niveau = this.props.plan_budgetaires.find(pl => pl.id == troisieme_niveau.parent)
                var cinquieme_niveau = this.props.plan_budgetaires.find(pl => pl.id == quatrieme_niveau.parent)
              ...
              axios.post('/api/modifier_plan_budgetaire/' + response.data.id, objet_a_envoyer).then(_ => {
                axios.get('/api/plan_budgetaires').then((response) => {
    ...
    onEditSubmitPlanEnfant = (e) => {
      ...
      this.setState({isFormSubmitted: true})
        axios.post('/api/modifier_plan_budgetaire/' + modif.id, {
          libelle: this.libelle_nouveau_plan_enfant_edit.value,
          code: this.code_nouveau_plan_enfant_edit.value,
    ...
    getFirstStructure = () => {
        return this.props.structure_budgetaires[0]
    }

      enregistrerPlanBudgetaire = async (e) => {
          e.preventDefault()
           ...
           await axios.post('/api/ajouter_plan_budgetaire',
               {
                   ...
                   structure_budgetaire_id: this.state.premier_niveau_structure_budgetaire.id,


            }
            ).then(response => {
              axios.post('/api/modifier_plan_budgetaire/' + response.data.id, {
    ...
    renderlist(){
          ...
          <tbody>
           {this.props.plan_budgetaires.map((st, index) =>
               <PlanBudgetaireItem
                key={st.id}
                index={index}
                item={st}
      ...
      renderTree(){
        ...
        return (
         ...
         <OptionsList
         options={this.props.plan_budgetaires.filter(plan => plan.parent == null)}
        ...
        return (
            ...
            <div className="main-card card" >
                       <div className="card-body ">
                           ...
                           <h5 className="card-title">Gestion du plan Budgétaire
                   ...
                   {/*        <span className="pull-right">
                          ...
                          <br />
                            ...
                            <div className="col-md-3">
                                    ...
                                    <input
                                       ref={code_nouveau_plan_enfant => this.code_nouveau_plan_enfant = code_nouveau_plan_enfant}
                                       onChange={this.setField}
                                        name="code_nouveau_plan_enfant"
                                        placeholder="code "
                                       type="text" className="form-control " />
                                    </div>
                            ...
                            <div className="col-md-3">
                                     <input
                                       ...
                                       onChange={this.setField}
                                        ...
                                        placeholder="type"Libéllé
                                       type="text" className="form-control" />
                                     </div>

                                     <div className="col-md-2">
                                    <input
                                       ref={code_nouveau_plan_enfant_edit => this.code_nouveau_plan_enfant_edit = code_nouveau_plan_enfant_edit}
                                       onChange={this.setField}
                                        name="code_nouveau_plan_enfant_edit"
                                        placeholder="code "
                                        defaultValue={this.state.code_nouveau_plan_enfant_edit}
                                       type="text" className="form-control " />
                                    </div>
            ...
            <div className="theme-settings__inner">
                <div className="scrollbar-container">
                    <div className="theme-settings__options-wrapper">
                        ...
                        <form ref={(ref) => this.formRef = ref} className="p-3" onChange={this.setField} onSubmit={this.enregistrerPlanBudgetaire}>
                            <br />
                                ...
                                <div className="position-relative form-group">
                                    ...
                                    readOnly
                                    value={this.state.premier_niveau_structure_budgetaire.libelle}
                                       type="text" className="form-control" />
                                    </div>

                                    <div className="position-relative form-group">
                                    <label htmlFor="exampleAddress" className="">Code</label>
                                    <input name="code"
...
export default connect(mapStateToProps)(PlanBudgetaire)
