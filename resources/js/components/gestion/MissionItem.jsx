...
class MissionItem extends Component {
    ...
    render() {
        ...
        return (
            ...
            <tr >
                <td className="sticky-col first-col" onDoubleClick={this.props.onEdit.bind(this, item.id)}>{item.numero_ordre_mission ? item.numero_ordre_mission : ''}</td>
                <td className="sticky-col second-col" onDoubleClick={this.props.onEdit.bind(this, item.id)}>{item.date_demande ? moment(item.date_demande).format('DD/MM/YYYY') : 'Non renseigné'}</td>

                <td className="sticky-col third-col" onDoubleClick={this.props.onEdit.bind(this, item.id)}>{item.heure_demande ? item.heure_demande.slice(0, 5) : ''}</td>
                <td className="sticky-col thour-col" onDoubleClick={this.props.onEdit.bind(this, item.id)}>{item.demandeur ? item.demandeur.nom : ''}</td>
